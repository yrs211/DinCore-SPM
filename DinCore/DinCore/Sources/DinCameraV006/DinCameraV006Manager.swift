...
public class DinCameraV006Manager: NSObject, DinHomeDeviceManagerProtocol {
    ...
    public var categoryID: String {
        DinCameraV006.categoryID
    }

    private var unsafeOperatorsDisposeBag: DisposeBag?

    private var unsafeCameraOperators: [DinCameraV006Operator]? = [] {
        didSet {
    ...
    var cameraOperators: [DinCameraV006Operator]? {
        var tempOperators: [DinCameraV006Operator]?
...
//        save()
    ...
    public func getDevices(onlyLoadCache: Bool, complete: DinHomeGetDevicesDiffBlock?) -> [DinHomeDeviceProtocol]? {
        ...
        // 这里不用 [weak self] 是需要在deinit前，把self retain count +1 ，以防在下面线程执行时，main线程dealloc
        let request = DinLiveStreamingRequest.getDevicesByAddtime(addTime:1, homeID: homeID, providers: [DinCameraV006.provider])
    ...
    public func addDevice(_ device: DinHomeDeviceProtocol, complete: @escaping ()->(), inSafeQueue: Bool = false) {
        guard let newOperator = device as? DinCameraV006Operator else {
            complete()
            return
        }
        if inSafeQueue {
    ...
    public func shouldHandleNotification(_ notification: [String : Any]) -> Bool {
        ...
        notification["provider"] as? String == DinCameraV006.provider &&
    ...
    private func getDevice(identifiable: String, completion: ((Bool, DinCameraV006Operator?) -> Void)?) {
        let request = DinLiveStreamingRequest.getDevices(homeID: homeID, providers: [DinCameraV006.provider])
        DinHttpProvider.request(request) { [weak self] result in
            guard let self = self else { return }
            guard
                let camerasArr = result?["list"] as? [[String: Any]],
                let newCamera = camerasArr
                    .compactMap({ DinCameraV006.deserialize(from: $0) })
            ...
            let result = DinCameraV006Operator(
...
extension DinCameraV006Manager {
    private func saveKey() -> String {
        "DinHome_\(homeID)_\(userID)_\(DinCameraV006.categoryID)"
    }

    private func saveTimeKey() -> String {
        "DinHome_\(homeID)_\(userID)_\(DinCameraV006.categoryID)_SaveTime"
    }

    private func cache(cameras: [DinCameraV006Operator]?) {
    ...
    private func getCache() -> [DinCameraV006Operator]? {
        guard let cacheCamerasStrings = DinCore.dataBase.hasValue(withKey: saveKey()) as? [String] else {
            return nil
        }
        var operators = [DinCameraV006Operator]()
        for cacheCamerasString in cacheCamerasStrings {
            if let cameraOperator = DinCameraV006Operator(deviceInfo: cacheCamerasString, homeID: homeID) {
...
extension DinCameraV006Manager{
    ...
    public func searchDeviceFromServer(withIDs deviceIDs: [String], info: Any?, complete: ((Bool, [DinHomeDeviceProtocol]?) -> Void)?) {
        let pids = deviceIDs.map { pid in
            ...
            item["pid"] = pid
            item["provider"] = DinCameraV006.provider
            return item
        }
    ...
    public func acquireTemporaryDevices(withIDs deviceIDs: [String], info: Any?, complete: ((Bool, [any DinHomeDeviceProtocol]?) -> Void)?) {
        ...
        DinHttpProvider.request(request) { [weak self] result in
            if let camerasArr = result?["list"] as? [[String: Any]] {
                var resultDevices: [DinCameraV006Operator] = []
                for cameraArr in camerasArr {
                    if let deviceID = cameraArr["pid"] as? String, let camera = DinCameraV006.deserialize(from: cameraArr) {
                        resultDevices.append(DinCameraV006Operator(liveStreamingControl: DinLiveStreamingControl(withCommunicationDevice: camera, belongsToHome: self?.homeID ?? "")))
                    }
                }
                complete?(true, resultDevices)
            } else {
    ...
    public func getNewDevices(complete: DinHomeDeviceManagerProtocol.GetNewDeviceCompletion?) {
        ...
        let request = DinLiveStreamingRequest.getDevicesByAddtime(addTime: lastAddTime, homeID: homeID, providers: [DinCameraV006.provider])
        DinHttpProvider.request(request) { [weak self] result in
            if let camerasArr = result?["list"] as? [[String: Any]] {
    ...
    private func updateLocalDevices(deviceDictArr: [[String: Any]],
        ...
        DinUser.homeDeviceInfoQueue.async(flags: .barrier) {
            ...
            var updatedDevices: [DinCameraV006Operator] = []
            var brandNewDevices: [DinCameraV006Operator] = []
            for deviceDict in deviceDictArr {
                if let deviceID = deviceDict["pid"] as? String {
                    ...
                    // 如果设备不存在，创建新设备
                    if !found {
                        if let camera = DinCameraV006.deserialize(from: deviceDict) {
                            let deivice = DinCameraV006Operator(liveStreamingControl: DinLiveStreamingControl(withCommunicationDevice: camera, belongsToHome: self.homeID))
