//
//  DinStorageBatteryPowerStoreManager.swift
//  DinCore
//
...
final
public class DinStorageBatteryPowerStoreManager: DinStorageBatteryManager {

    public override var categoryID: String { DinStorageBatteryPowerStore.categoryID }

    override var model: String { DinStorageBatteryPowerStore.provider }

    override var cacheKey: String { "DinHom_\(homeID)_\(userID)_\(DinStorageBatteryPowerStore.categoryID)" }
    
    override var saveTimeKey: String { "DinHom_\(homeID)_\(userID)_\(DinStorageBatteryPowerStore.categoryID)_SaveTime" }
    ...
    override func createModel(from rawData: [String : Any]) -> DinStorageBattery? {
        DinStorageBatteryPowerStore.deserialize(from: rawData)
    }

    override func createControl(from model: DinStorageBattery, homeID: String) -> DinStorageBatteryControl {
        guard model is DinStorageBatteryPowerStore else { fatalError("Incorrect type of model") }
        let result = DinStorageBatteryPowerStoreControl(model: model, homeID: homeID)
        return result
    }

    override func createOperator(from control: DinStorageBatteryControl) -> DinStorageBatteryOperator {
        guard control is DinStorageBatteryPowerStoreControl else { fatalError("Incorrect type of control") }
        let result = DinStorageBatteryPowerStoreOperator(control: control)
        return result
    }

    public override func addDevice(_ device: DinHomeDeviceProtocol, complete: @escaping ()->(), inSafeQueue: Bool = false) {
        guard let `operator` = device as? DinStorageBatteryPowerStoreOperator else { return }
    ...
    override func createOperator(from rawString: String, homeID: String) -> DinStorageBatteryOperator? {
        DinStorageBatteryPowerStoreOperator(homeID: homeID, rawString: rawString)
    }
    ...
    public override func shouldHandleNotification(_ notification: [String : Any]) -> Bool {
        ...
        notification["model"] as? String == DinStorageBatteryPowerStore.provider &&
