...
public protocol DinIPCPeripheralDelegate: AnyObject {
    func dinIPCPeripheral(
        _ peripheral: DinIPCPeripheral,
        completeCommand token: DinIPCPeripheral.ExecutionToken,
        didReceiveResponse result: Result<DinIPCPeripheral.Command.Response, DinIPCPeripheral.CommandExecutionError>
    )
}

/// 扫描到的 IPC 设备
final public class DinIPCPeripheral: DinPeripheral {

    public weak var delegate: DinIPCPeripheralDelegate?
    ...
    private func handleNotifiedUpdate(data: Data) {
        ...
        if #available(iOS 14.0, *) {
            DinCoreLogger.bluetoothCommunication.debug("receive DataString - \(response.toJSONString() ?? "")")
        }
#endif
...
#if ENABLE_LOG
        if #available(iOS 14.0, *) {
            DinCoreLogger.bluetoothCommunication.debug("send DataString - \(jsonString)")
        }
#endif
        executionToken = nil
...
#if ENABLE_LOG
            if #available(iOS 14.0, *) {
                DinCoreLogger.bluetoothCommunication.debug("\(token.command.rawValue) task timeout")
            }
#endif
            self.executionToken = nil
            self.delegate?.dinIPCPeripheral(
                self,
                completeCommand: token,
...
extension DinIPCPeripheral: CBPeripheralDelegate {
...
extension DinIPCPeripheral.Command {
