...
class DinNovaKeypadControl: DinNovaPluginControl {
...
extension DinNovaKeypadControl {
    override func updatePluginInfo(complete: (()->())?) {
        super.updatePluginInfo { [weak self] in
            self?.updateSendID((self?.plugin as? DinNovaKeypad)?.sendid ?? "", usingSync: false, complete: complete)
        }
    }

    func updateSendID(_ sendid: String, usingSync sync: Bool, complete: (()->())?) {
        if sync {
            (plugin as? DinNovaKeypad)?.sendid = sendid
            unsafeInfo["sendid"] = sendid
        } else {
            DinUser.homeDeviceInfoQueue.async(flags: .barrier) { [weak self] in
                guard let self = self else { return }
                (self.plugin as? DinNovaKeypad)?.sendid = sendid
