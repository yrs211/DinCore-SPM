//
//  DinStorageBatteryPowerCore20Manager.swift
//  DinCore
//
...
final
public class DinStorageBatteryPowerCore20Manager: DinStorageBatteryManager {

    public override var categoryID: String { DinStorageBatteryPowerCore20.categoryID }

    override var model: String { DinStorageBatteryPowerCore20.provider }

    override var cacheKey: String { "DinHom_\(homeID)_\(userID)_\(DinStorageBatteryPowerCore20.categoryID)" }
    
    override var saveTimeKey: String { "DinHom_\(homeID)_\(userID)_\(DinStorageBatteryPowerCore20.categoryID)_SaveTime" }
    ...
    override func createModel(from rawData: [String : Any]) -> DinStorageBattery? {
        DinStorageBatteryPowerCore20.deserialize(from: rawData)
    }

    override func createControl(from model: DinStorageBattery, homeID: String) -> DinStorageBatteryControl {
        guard model is DinStorageBatteryPowerCore20 else { fatalError("Incorrect type of model") }
        let result = DinStorageBatteryPowerCore20Control(model: model, homeID: homeID)
        return result
    }

    override func createOperator(from control: DinStorageBatteryControl) -> DinStorageBatteryOperator {
        guard control is DinStorageBatteryPowerCore20Control else { fatalError("Incorrect type of control") }
        let result = DinStorageBatteryPowerCore20Operator(control: control)
        return result
    }

    public override func addDevice(_ device: DinHomeDeviceProtocol, complete: @escaping ()->(), inSafeQueue: Bool = false) {
        guard let `operator` = device as? DinStorageBatteryPowerCore20Operator else { return }
    ...
    override func createOperator(from rawString: String, homeID: String) -> DinStorageBatteryOperator? {
        DinStorageBatteryPowerCore20Operator(homeID: homeID, rawString: rawString)
    }
    ...
    public override func shouldHandleNotification(_ notification: [String : Any]) -> Bool {
        ...
        notification["model"] as? String == DinStorageBatteryPowerCore20.provider &&
