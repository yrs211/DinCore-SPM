...
public class DinCameraV015Manager: NSObject, DinHomeDeviceManagerProtocol {
    ...
    public var categoryID: String {
        DinCameraV015.categoryID
    }

    private var unsafeOperatorsDisposeBag: DisposeBag?

    private var unsafeCameraOperators: [DinCameraV015Operator]? = [] {
        didSet {
    ...
    var cameraOperators: [DinCameraV015Operator]? {
        var tempOperators: [DinCameraV015Operator]?
...
//        save()
    ...
    public func getDevices(onlyLoadCache: Bool, complete: DinHomeGetDevicesDiffBlock?) -> [DinHomeDeviceProtocol]? {
        ...
        // 这里不用 [weak self] 是需要在deinit前，把self retain count +1 ，以防在下面线程执行时，main线程dealloc
        let request = DinLiveStreamingRequest.getDevicesByAddtime(addTime:1, homeID: homeID, providers: [DinCameraV015.provider])
        DinHttpProvider.request(request) { result in
            if let camerasArr = result?["list"] as? [[String: Any]] {
                // 这里不用 [weak self] 是需要在deinit前，把self retain count +1 ，以防在下面线程执行时，main线程dealloc
                self.updateLocalDevices(deviceDictArr: camerasArr, complete: { _, addedDevices in
                    DispatchQueue.main.async {
                        self.save()
    ...
    public func addDevice(_ device: DinHomeDeviceProtocol, complete: @escaping ()->(), inSafeQueue: Bool = false) {
        guard let newOperator = device as? DinCameraV015Operator else {
            complete()
            return
        }
        if inSafeQueue {
    ...
    public func shouldHandleNotification(_ notification: [String : Any]) -> Bool {
        ...
        notification["provider"] as? String == DinCameraV015.provider &&
    ...
    private func getDevice(identifiable: String, completion: ((Bool, DinCameraV015Operator?) -> Void)?) {
        let request = DinLiveStreamingRequest.getDevices(homeID: homeID, providers: [DinCameraV015.provider])
        DinHttpProvider.request(request) { [weak self] result in
            guard let self = self else { return }
            guard
                let camerasArr = result?["list"] as? [[String: Any]],
                let newCamera = camerasArr
                    .compactMap({ DinCameraV015.deserialize(from: $0) })
            ...
            let result = DinCameraV015Operator(
...
extension DinCameraV015Manager {
    private func saveKey() -> String {
        "DinHome_\(homeID)_\(userID)_\(DinCameraV015.categoryID)"
    }
    
    private func saveTimeKey() -> String {
        "DinHome_\(homeID)_\(userID)_\(DinCameraV015.categoryID)_SaveTime"
    }

    private func cache(cameras: [DinCameraV015Operator]?) {
    ...
    private func getCache() -> [DinCameraV015Operator]? {
        guard let cacheCamerasStrings = DinCore.dataBase.hasValue(withKey: saveKey()) as? [String] else {
            return nil
        }
        var operators = [DinCameraV015Operator]()
        for cacheCamerasString in cacheCamerasStrings {
            if let cameraOperator = DinCameraV015Operator(deviceInfo: cacheCamerasString, homeID: homeID) {
...
extension DinCameraV015Manager {
    ...
    public func searchDeviceFromServer(withIDs deviceIDs: [String], info: Any?, complete: ((Bool, [DinHomeDeviceProtocol]?) -> Void)?) {
        let pids = deviceIDs.map { pid in
            ...
            item["pid"] = pid
            item["provider"] = DinCameraV015.provider
            return item
        }
    ...
    public func acquireTemporaryDevices(withIDs deviceIDs: [String], info: Any?, complete: ((Bool, [any DinHomeDeviceProtocol]?) -> Void)?) {
        ...
        DinHttpProvider.request(request) { [weak self] result in
            if let camerasArr = result?["list"] as? [[String: Any]] {
                var resultDevices: [DinCameraV015Operator] = []
                for cameraArr in camerasArr {
                    if let deviceID = cameraArr["pid"] as? String, let camera = DinCameraV015.deserialize(from: cameraArr) {
                        resultDevices.append(DinCameraV015Operator(liveStreamingControl: DinLiveStreamingControl(withCommunicationDevice: camera, belongsToHome: self?.homeID ?? "")))
                    }
                }
                complete?(true, resultDevices)
            } else {
    ...
    public func getNewDevices(complete: DinHomeDeviceManagerProtocol.GetNewDeviceCompletion?) {
        ...
        let request = DinLiveStreamingRequest.getDevicesByAddtime(addTime: lastAddTime, homeID: homeID, providers: [DinCameraV015.provider])
        DinHttpProvider.request(request) { [weak self] result in
            if let camerasArr = result?["list"] as? [[String: Any]] {
    ...
    private func updateLocalDevices(deviceDictArr: [[String: Any]],
        ...
        DinUser.homeDeviceInfoQueue.async(flags: .barrier) {
            ...
            var updatedDevices: [DinCameraV015Operator] = []
            var brandNewDevices: [DinCameraV015Operator] = []
            ...
            for deviceDict in deviceDictArr {
                if let deviceID = deviceDict["pid"] as? String {
                    ...
                    if !found {
                        if let camera = DinCameraV015.deserialize(from: deviceDict) {
                            let deivice = DinCameraV015Operator(liveStreamingControl: DinLiveStreamingControl(withCommunicationDevice: camera, belongsToHome: self.homeID))
    ...
    private func updateLocalDevices(deviceIDs: [String],
        ...
        updateLocalDevices(deviceDictArr: deviceDictArr) { updatedDevices, brandNewDevices in
            ...
            DinUser.homeDeviceInfoQueue.async(flags: .barrier) {
                ...
                // Set device in the local device list to be deleted
