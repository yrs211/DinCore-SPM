//
//  DinStorageBatteryHP5000Manager.swift
//  DinCore
//
...
final
public class DinStorageBatteryHP5000Manager: DinStorageBatteryManager {

    public override var categoryID: String { DinStorageBatteryHP5000.categoryID }

    override var model: String { DinStorageBatteryHP5000.provider }

    override var cacheKey: String { "DinHom_\(homeID)_\(userID)_\(DinStorageBatteryHP5000.categoryID)" }
    
    override var saveTimeKey: String { "DinHom_\(homeID)_\(userID)_\(DinStorageBatteryHP5000.categoryID)_SaveTime" }
    ...
    override func createModel(from rawData: [String : Any]) -> DinStorageBattery? {
        DinStorageBatteryHP5000.deserialize(from: rawData)
    }

    override func createControl(from model: DinStorageBattery, homeID: String) -> DinStorageBatteryControl {
        guard model is DinStorageBatteryHP5000 else { fatalError("Incorrect type of model") }
        let result = DinStorageBatteryHP5000Control(model: model, homeID: homeID)
        return result
    }

    override func createOperator(from control: DinStorageBatteryControl) -> DinStorageBatteryOperator {
        guard control is DinStorageBatteryHP5000Control else { fatalError("Incorrect type of control") }
        let result = DinStorageBatteryHP5000Operator(control: control)
        return result
    }
    
    public override func addDevice(_ device: DinHomeDeviceProtocol, complete: @escaping ()->(), inSafeQueue: Bool = false) {
        guard let `operator` = device as? DinStorageBatteryHP5000Operator else { return }
    ...
    override func createOperator(from rawString: String, homeID: String) -> DinStorageBatteryOperator? {
        DinStorageBatteryHP5000Operator(homeID: homeID, rawString: rawString)
    }
    ...
    public override func shouldHandleNotification(_ notification: [String : Any]) -> Bool {
        ...
        notification["model"] as? String == DinStorageBatteryHP5000.provider &&
